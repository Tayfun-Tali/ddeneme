...
class DataAnalyzerApp:
    def __init__(self, master):
        ...
        # --- NİHAİ MİMARİ ---
        self.original_df = None         # Tüm veriyi tutan TEK DataFrame.
        self.prediction_df = None
        self.plotted_sgs = []           # Grafikte hangi SG'lerin çizileceğini tutan liste.
        self.is_view_trimmed = False    # Görünümün kesilmiş olup olmadığını tutan durum.
    ...
    def get_display_df(self):
        ...
        if self.is_view_trimmed:
            ...
            if load_column and load_column in self.original_df.columns:
                ...
                except ValueError: # Eğer hiç yük yoksa veya hepsi NaN ise
                    return self.original_df
        return self.original_df

    def create_widgets(self):
        # Bu fonksiyonda değişiklik yok, tamlık için eklendi.
        ...
        self.fig, self.ax = plt.subplots(dpi=100); self.ax.grid(True)
    ...
    # ### YENİ VE NİHAİ YAKLAŞIM: "TEK ÇİZİM NOKTASI" ###
    # Bu fonksiyon, grafiği çizmekten sorumlu TEK yerdir.
    def _redraw_all_plots(self):
        ...
        # 3. `plotted_sgs` listesindeki her şeyi bu veriyle yeniden çiz
        ...
        # 4. Grafiği ayarla ve güncelle
        self.ax.relim(); self.ax.autoscale_view()
        if self.plotted_sgs or self.prediction_df is not None:
            self.ax.legend()
        else:
            self.ax.set_title(f"Yük Oranına Karşı Strain ({self.combo_id.get()})")
            self.ax.set_xlabel("Yük Oranı (%)"); self.ax.set_ylabel("Strain (μstrain)")
        self.canvas.draw()

    # ### YENİ VE NİHAİ YAKLAŞIM: FONKSİYONLAR SADECE DURUMU DEĞİŞTİRİR ###
    # Bu fonksiyon artık çizim yapmıyor, sadece listeyi güncelleyip "usta çizer"i çağırıyor.
    def grafige_ekle(self):
        selected_sg = self.combo_sg.get()
        if self.original_df is None or not selected_sg: return
        if selected_sg in self.plotted_sgs: return
        
        # 1. Durumu Değiştir: Listeye ekle
        ...
        self.lbl_durum.config(text=f"'{selected_sg}' eklendi.")
        self.notebook.select(0)

    # Bu fonksiyon da sadece listeyi güncelleyip "usta çizer"i çağırıyor.
    def grafigden_cikar(self):
        selected_sg = self.combo_sg.get()
        if not selected_sg or selected_sg not in self.plotted_sgs: return
        
        # 1. Durumu Değiştir: Listeden çıkar
    ...
    # Butonlar sadece durumu (`is_view_trimmed`) değiştirip "usta çizer"i çağırıyor.
    ...
    def perform_calculation(self, calc_name):
        # Bu fonksiyon, hesaplamayı yapıp `original_df`'i günceller ve SONUNDA "usta çizer"i çağırır.
        if self.original_df is None: return
        ...
        if calculated_count > 0:
            ...
            self.filtrele_sg()
            self._redraw_all_plots() # Hesaplama sonrası "usta çizer" çağrılır.
    ...
    # --- Diğer Fonksiyonlar (Bu yeni mimariyle uyumlu hale getirildi) ---
    ...
    def grafigi_temizle(self):
        ...
        self.is_view_trimmed = False
        self._redraw_all_plots()
        self.ax.set_title("Grafik Temizlendi") # Redraw'dan sonra başlığı ayarla
        self.canvas.draw()
    ...
    def sg_secildi(self, event=None):
        ...
        state_plus = "disabled" if selected_sg in self.plotted_sgs else "normal"
        state_minus = "normal" if selected_sg in self.plotted_sgs else "disabled"
        self.btn_plus.config(state=state_plus); self.btn_minus.config(state=state_minus)
        try:
            load_column = self._get_load_column()
            if load_column and selected_sg in self.original_df.columns:
                self.guncelle_tablo(self.original_df[[load_column, selected_sg]])
                self.lbl_durum.config(text=f"'{selected_sg}' seçildi.")
        except Exception as e:
            messagebox.showerror("Tablo Hatası", f"Tablo oluşturulurken hata: {e}"); self.guncelle_tablo(None)
    ...
    def on_hover(self, event):
        ...
        if closest_point:
            ...
            if xlim[1] == xlim[0] or ylim[1] == ylim[0]: return # Sıfıra bölünme hatasını engelle
