...
class DataAnalyzerApp:
    def __init__(self, master):
        ...
        # YENİ: Orjinal, tam DataFrame'i saklamak için
    ...
    def create_widgets(self):
        ...
        # ... (Diğer widget'lar öncekiyle aynı) ...
        ...
        lbl_sg = ttk.Label(kontrol_cerceve, text="Strain Gauge Seç:")
        lbl_sg.grid(row=3, column=2, padx=(10, 5), pady=5, sticky="w")
        self.combo_sg = ttk.Combobox(kontrol_cerceve, state="readonly", width=30)
        self.combo_sg.grid(row=3, column=3, padx=5, pady=5, sticky="ew")
        ...
        self.btn_tahmin.grid(row=4, column=0, columnspan=2, padx=5, pady=10, sticky="ew")
        self.btn_ekle = ttk.Button(kontrol_cerceve, text="GRAFİĞE EKLE", command=self.grafige_ekle)
        self.btn_ekle.grid(row=4, column=2, padx=5, pady=10, sticky="ew")
        self.btn_temizle = ttk.Button(kontrol_cerceve, text="GRAFİĞİ TEMİZLE", command=self.grafigi_temizle)
        self.btn_temizle.grid(row=4, column=3, padx=5, pady=10, sticky="ew")
        
        # --- DEĞİŞİKLİK: Yeni Butonlar Eklendi ---
        ...
        # ... (Notebook ve diğer widget'lar aynı kalıyor) ...
    ...
    def sadece_yuklemeyi_goster(self):
        ...
        try:
            # Yük sütunundaki maksimum değerin ilk bulunduğu satırın indeksini al
            max_load_index = self.original_df['Load_Ratio_MON1'].idxmax()
            
            # Veriyi bu indekse kadar kes
            self.current_df = self.original_df.loc[:max_load_index].copy()
            
            # Grafiği kesilmiş veriyle yeniden çiz
    ...
    def tum_veriyi_goster(self):
        ...
        # Mevcut veriyi orijinal, tam veriyle değiştir
        self.current_df = self.original_df.copy()
        
        # Grafiği tam veriyle yeniden çiz
        self._redraw_all_plots()
        self.lbl_durum.config(text="Tüm veri aralığı gösteriliyor.")
        
    def _redraw_all_plots(self):
        """Grafikteki tüm çizgileri self.current_df'e göre yeniden çizen yardımcı metod."""
        ...
        # Daha önce çizilmiş tüm strain gauge'leri yeni veriyle tekrar çiz
        ...
        # Tahmin verisi varsa onu da çiz
    ...
    def grafige_ekle(self):
        if self.current_df is None or self.combo_sg.get() == "":
            messagebox.showwarning("Eksik Bilgi", "Lütfen veri yükleyip bir Strain Gauge seçin.")
            return
        ...
        try:
            ...
            x_column = 'Load_Ratio_MON1'
            self.ax.plot(self.current_df[x_column], self.current_df[selected_sg], 
                         marker='o', linestyle='-', label=selected_sg)
            
            self.plotted_sgs.append(selected_sg)
            
            # DEĞİŞİKLİK: Butonları aktifleştir
            self.btn_trim.config(state="normal")
            self.btn_reset_view.config(state="normal")

            self.ax.legend()
            self.canvas.draw()
            self.lbl_durum.config(text=f"'{selected_sg}' grafiğe eklendi. ({len(self.plotted_sgs)} adet)")
    ...
    def grafigi_temizle(self):
        ...
        self.prediction_df = None # Tahmin verisini de temizle
        ...
        # DEĞİŞİKLİK: Butonları pasifleştir
    ...
    def id_secildi(self, event=None):
        ...
        try:
            ...
            # DEĞİŞİKLİK: Okunan veriyi hem current hem de original df'e ata
            ...
            for col in self.current_df.columns:
                self.current_df[col] = pd.to_numeric(self.current_df[col], errors='ignore')
            
            self.original_df = self.current_df.copy() # Orijinal veriyi yedekle
    ...
    # ... (Diğer tüm metodlar aynı kalabilir) ...
    ...
    def sg_secildi(self, event=None):
        selected_sg = self.combo_sg.get()
        if not selected_sg or self.current_df is None: return
        try:
            load_column = 'Load_Ratio_MON1'
            columns_to_display = [load_column, selected_sg]
            existing_columns = [c for c in columns_to_display if c in self.current_df.columns]
            if len(existing_columns) != 2:
                messagebox.showwarning("Sütun Bulunamadı", f"'{load_column}' veya '{selected_sg}' sütunlarından biri veride bulunamadı.")
                return
            
            # DEĞİŞİKLİK: Tabloyu kesilmiş/tam veriye göre değil, her zaman orijinal veriden al
            display_df = self.original_df[existing_columns]
            self.guncelle_tablo(display_df)
            self.lbl_durum.config(text=f"'{selected_sg}' seçildi. Grafiğe ekleyebilir veya verisini inceleyebilirsiniz.")
        except Exception as e:
...
class DataAnalyzerApp:
    ...
    def create_widgets(self):
        ...
        # --- DEĞİŞİKLİK: SG Seçim ve Ekle/Çıkar butonları için yeni Frame ---
        ...
        # --- Butonların Konumları Güncellendi ---
        self.btn_tahmin = ttk.Button(kontrol_cerceve, text="Tahmin Verisi Yükle (.dat)", command=self.tahmin_verisi_yukle)
        self.btn_tahmin.grid(row=4, column=0, columnspan=2, padx=5, pady=10, sticky="ew")
    ...
    def grafige_ekle(self):
        ...
        try:
            ...
            self.btn_trim.config(state="normal")
            self.btn_reset_view.config(state="normal")
            self.sg_secildi() # Buton durumlarını güncellemek için çağır
    ...
    # YENİ: Seçili strain gauge'ü grafikten çıkaran metod
    def grafigden_cikar(self):
        ...
        # Grafikteki ilgili çizgiyi bul ve kaldır
        ...
        if self.plotted_sgs:
            ...
            # Son çizgi de kaldırıldıysa legend'i tamamen sil
            if self.ax.get_legend() is not None:
                self.ax.get_legend().remove()
        
        self.sg_secildi() # Buton durumlarını güncelle
        self.canvas.draw()
    ...
    def grafigi_temizle(self):
        ...
        # Tüm kontrol butonlarını pasifleştir
        self.btn_trim.config(state="disabled")
        self.btn_reset_view.config(state="disabled")
    ...
    def sg_secildi(self, event=None):
        ...
        # DEĞİŞİKLİK: +/- butonlarının durumunu yönet
        if selected_sg in self.plotted_sgs:
            self.btn_plus.config(state="disabled")
