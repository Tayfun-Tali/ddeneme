...
class DataAnalyzerApp:
    def __init__(self, master):
        ...
        # --- DEĞİŞİKLİK: Daha Sağlam ve Anlaşılır Hesaplama Kütüphanesi ---
        self.calculations = {
            "Shear (S = 2B - A - C)": {
                "inputs": ['A', 'B', 'C'],
                "output_suffix": 'S',
                # Lambda parametreleri artık girdi isimleriyle aynı. Bu sayede sıra önemli değil.
    ...
    # --- DEĞİŞİKLİK: Bu fonksiyon tamamen yenilendi ve çok daha güvenli hale getirildi ---
    def perform_calculation(self, calc_name):
        ...
        groups = {}
        # Sütunları gruplara ayır (artık '_' ile ayrılmış isimler üzerinden çalışıyor)
        for sg_name in self.all_sg_columns:
            if "_" in sg_name:
                base_name = sg_name.split('_')[0]
                if len(base_name) > 1:
                    prefix = base_name[:-1]
                    letter = base_name[-1]
        ...
        for prefix, gauges in groups.items():
            if all(inp in gauges for inp in required_inputs):
                new_col_name = f"{prefix}{output_suffix}_MON1"
                if new_col_name in self.original_df.columns:
                    continue
                
                # Girdileri bir sözlük olarak hazırla. Bu, formülün isme göre erişmesini sağlar.
                input_data = {inp: self.original_df[gauges[inp]] for inp in required_inputs}
                
                # Formülü, anahtar kelime argümanları (**input_data) ile güvenli bir şekilde çağır.
    ...
    # --- DEĞİŞİKLİK: Sütun isimleri okunduğu anda değiştiriliyor ve liste de güncelleniyor ---
    def id_secildi(self, event=None):
        ...
        try:
            ...
            original_sg_columns = [h for h, u in zip(header_row, unit_row) if u == 'μstrain']
            
            self.current_df = pd.read_csv(filepath, sep=r'\s+', header=0, skiprows=[1], engine='python')
            
            # DataFrame'deki sütun isimlerini değiştir
            self.current_df.columns = self.current_df.columns.str.replace(':', '_', regex=False)
            
            # Sütun listemizi de DataFrame ile tutarlı hale getir
            sg_columns = [col.replace(':', '_', 1) for col in original_sg_columns]
            ...
            self.all_sg_columns = sg_columns # Artık alt çizgili isimleri tutuyoruz
