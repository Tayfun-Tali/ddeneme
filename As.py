...
class DataAnalyzerApp:
    def __init__(self, master):
        ...
        # YENİ: Hesaplama için önceden işlenmiş ve filtrelenmiş gruplar
        self.shear_rosettes = {}
        self.average_pairs = {}
    ...
    # --- PERFORMANS İYİLEŞTİRMESİ: GRUPLAMA İÇİN YARDIMCI FONKSİYON ---
    def _tespit_et_hesaplama_gruplarini(self):
        """
        Tüm fiziksel SG'leri bir kez tarayarak hesaplamalar için
        gerekli grupları (rosette'ler, D/E çiftleri vs.) tespit eder ve saklar.
        Bu fonksiyon sadece veri dosyası yüklendiğinde bir kez çalışır.
        """
        self.shear_rosettes = {}
        self.average_pairs = {}
    ...
    def perform_calculation(self, calc_name):
        ...
        # Hangi önceden hazırlanmış grup listesinin kullanılacağını seç
        ...
        # Sadece önceden tespit edilmiş hazır gruplar üzerinde anında çalış
        for prefix, gauges in gruplar.items():
            ...
            try:
                # Gerekli girdileri hazırla (Örn: A, B, C veya D, E)
                input_data = {inp: self.original_df[gauges[inp]] for inp in calculation["inputs"]}
                
                # Formülü uygula ve ana (orijinal) DataFrame'e ekle
        ...
        if calculated_count > 0:
            # GÜVENLİ GÜNCELLEME: current_df'i, ana df'in son haline göre yeniden oluştur
            ...
            self.filtrele_sg()
            self._redraw_all_plots() # Grafiği de yeni verilerle güncelle
    ...
    def id_secildi(self, event=None):
        ...
        try:
            # Not: Ayraç (sep) olarak '\s+' kullanmak, tab ve boşlukları daha esnek yönetir.
            ...
            # Sayısal olmayan verilerde hata vermemesi için 'errors' parametresi eklendi.
            ...
            # --- YENİ YAKLAŞIM: Grupları en başta SADECE BİR KEZ tespit et ---
    ...
    def grafigi_temizle(self):
        ...
        self.prediction_df = None
        self.ax.clear() # Daha temiz bir yöntem
        self.ax.grid(True)
        ...
        if hasattr(self, 'calculate_menubutton'): self.calculate_menubutton.config(state="normal" if self.original_df is not None else "disabled")
        self.canvas.draw()
    ...
    def on_hover(self, event):
        ...
        if closest_point:
            ...
            # Toleransı veri aralığına göre dinamik yap
    ...
    def grafige_ekle(self):
        ...
        # Not: Sütun adının 'Load_Ratio:MON1' veya 'Load_Ratio_MON1' olduğundan emin olun
        # Veri dosyanızdaki gerçek isme göre bu satırı düzenlemeniz gerekebilir.
        x_column = next((col for col in self.current_df.columns if 'Load_Ratio' in col), None)
        if x_column is None:
            messagebox.showerror("Hata", "Yük verisi sütunu ('Load_Ratio') bulunamadı.")
            return
    ...
    def grafigden_cikar(self):
        ...
        # Çizgiyi etiketine göre bul ve kaldır
        ...
        if self.plotted_sgs:
            self.ax.legend()
        else: # Eğer hiç çizgi kalmadıysa legend'ı da kaldır
    ...
    def _get_load_column(self):
        """DataFrame içindeki yük sütununun adını güvenli bir şekilde bulur."""
    ...
    def grafik_popup(self):
        ...
        annot_popup = ax_popup.annotate("", xy=(0, 0), xytext=(20, 20), textcoords="offset points",
                                         bbox=dict(boxstyle="round", fc="yellow", alpha=0.7),
                                         arrowprops=dict(arrowstyle="->"))
    ...
    def on_popup_hover(self, event):
        ...
        # Diğer on_hover ile aynı mantık
    ...
    def dosya_sec(self):
        filepath = filedialog.askopenfilename(
            title="Bir .dat veri dosyası seçin",
            filetypes=(("Veri Dosyaları", "*.dat"), ("Tüm Dosyalar", "*.*")))
    ...
    def tahmin_verisi_yukle(self):
        ...
        path = filedialog.askopenfilename(
            title="Tahmin Değerlerini İçeren .dat Dosyasını Seçin",
            filetypes=(("DAT Dosyaları", "*.dat"), ("Tüm Dosyalar", "*.*")))
        if not path: return
        ...
        try:
            ...
            self._redraw_all_plots() # Tahmin verisini ekleyip grafiği yeniden çiz
