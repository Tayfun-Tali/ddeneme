...
import numpy as np
import re # YENİ: Regular Expression modülünü import et
...
class DataAnalyzerApp:
    ...
    # --- DEĞİŞİKLİK: Ayrıştırma mantığı Regex ile güncellendi ---
    def perform_calculation(self, calc_name):
        ...
        groups = {}
        # Regex deseni: sonunda (bir veya daha fazla rakam)(\sadece bir büyük harf) olanları bul
        # Örnek: '1020A' -> ('1020', 'A') olarak ayırır
        ...
        for sg_name in self.physical_sg_columns:
            if ":" in sg_name:
                ...
                if match:
                    prefix = match.group(1) # Rakam kısmı (örn: '1020')
                    letter = match.group(2) # Harf kısmı (örn: 'A')
                    
                    if prefix not in groups:
                        groups[prefix] = {"suffix": suffix_part}
