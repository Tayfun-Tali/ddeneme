...
class DataAnalyzerApp:
    ...
    # --- DEĞİŞİKLİK: İsim ayrıştırma mantığı güncellendi ---
    def perform_calculation(self, calc_name):
        ...
        for sg_name in original_sg_columns:
            if ":" in sg_name:
                base_name = sg_name.split(':')[0] # Örnek: '1001A'
                suffix = sg_name.split(':')[1]  # Örnek: 'MON1'
                if len(base_name) > 1:
                    prefix = base_name[:-1]      # Örnek: '1001'
                    letter = base_name[-1]       # Örnek: 'A'
                    if prefix not in groups:
                        groups[prefix] = {"suffix": suffix} # MON1 bilgisini sakla
    ...
    # --- DEĞİŞİKLİK: İsim değiştirme (`replace`) işlemi kaldırıldı ---
    def id_secildi(self, event=None):
        ...
        try:
            ...
            # İSİM DEĞİŞTİRME SATIRLARI KALDIRILDI. Orjinaller kullanılıyor.
            ...
            self.all_sg_columns = sg_columns # Orjinal isimleri sakla
