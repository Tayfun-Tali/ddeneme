...
class DataAnalyzerApp:
    def __init__(self, master):
        ...
        # Genişletilebilir Hesaplama Kütüphanesi
        self.calculations = {
            "Shear (S = 2B - A - C)": {
                "inputs": ['A', 'B', 'C'], # Gerekli sensör son ekleri
                "output_suffix": 'S',       # Çıktı sensörünün son eki
                "formula": lambda a, b, c: 2 * b - a - c # Uygulanacak formül
            },
            # Buraya gelecekte "Average (Avg = (D+E)/2)" gibi yeni formüller eklenebilir
        }
    ...
    def perform_calculation(self, calc_name):
        ...
        groups = {}
        # Sadece orijinal, dosyadan gelen strain gauge'leri tara
        # Bu, daha önce hesaplanmış sanal sensörleri tekrar taramayı önler
        original_sg_columns = [col for col in self.all_sg_columns if not any(col.endswith(f"{c['output_suffix']}:MON1") for c in self.calculations.values())]
